...
export const useCommunityPosts=(page:number,q='',category='')=>useQuery({queryKey:[...communityKeys.posts,page,q,category],queryFn:()=>q||category?api.listPosts(page,20,q,category):api.listPosts(page)});
...
export function useCreateCommunityPost(){const qc=useQueryClient();return useMutation({mutationFn:(input:Parameters<typeof api.createPost>[0])=>api.createPost(input),onSuccess:()=>qc.invalidateQueries({queryKey:communityKeys.posts})})}
